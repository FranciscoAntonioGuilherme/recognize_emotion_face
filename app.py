...
def load_images_from_folder(folder):
    ...
    # Percorrer todos os arquivos na pasta images
...
def detect_faces_and_emotions(video_path, output_path, known_face_encodings, known_face_names):
    ...
    fourcc = cv2.VideoWriter_fourcc(*'mp4v') # Codev para MP4
    out = cv2.VideoWriter(output_path, fourcc, fps, (width, height))

    # Loop para processar cada frame do video com barra de progresso
    for _ in tqdm(range(total_frames), desc="Processando vídeo"):
        # Ler um frame do vídeo
        ret, frame = cap.read()

        # Se não conseguil ler o frame (final do vídeo), sair do loop
        if not ret:
            break

        # Analizar o frame para detectar faces e expressões
        ...
        for face in result:
            ...
            cv2.putText(frame, dominant_emotion, (x+120, y-10), cv2.FONT_HERSHEY_SIMPLEX, 0.9, (36, 255, 12), 2)

            # Associar a face detectada pelo DeepFace com as faces conhecidas
            for (top, rigth, botton, left), name in zip(face_locations, face_names):
